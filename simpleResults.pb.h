// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: simpleResults.proto

#ifndef PROTOBUF_INCLUDED_simpleResults_2eproto
#define PROTOBUF_INCLUDED_simpleResults_2eproto

#include <string>

#include <google/protobuf/stubs/common.h>

#if GOOGLE_PROTOBUF_VERSION < 3006001
#error This file was generated by a newer version of protoc which is
#error incompatible with your Protocol Buffer headers.  Please update
#error your headers.
#endif
#if 3006001 < GOOGLE_PROTOBUF_MIN_PROTOC_VERSION
#error This file was generated by an older version of protoc which is
#error incompatible with your Protocol Buffer headers.  Please
#error regenerate this file with a newer version of protoc.
#endif

#include <google/protobuf/io/coded_stream.h>
#include <google/protobuf/arena.h>
#include <google/protobuf/arenastring.h>
#include <google/protobuf/generated_message_table_driven.h>
#include <google/protobuf/generated_message_util.h>
#include <google/protobuf/inlined_string_field.h>
#include <google/protobuf/metadata.h>
#include <google/protobuf/message.h>
#include <google/protobuf/repeated_field.h>  // IWYU pragma: export
#include <google/protobuf/extension_set.h>  // IWYU pragma: export
#include <google/protobuf/unknown_field_set.h>
// @@protoc_insertion_point(includes)
#define PROTOBUF_INTERNAL_EXPORT_protobuf_simpleResults_2eproto 

namespace protobuf_simpleResults_2eproto {
// Internal implementation detail -- do not use these members.
struct TableStruct {
  static const ::google::protobuf::internal::ParseTableField entries[];
  static const ::google::protobuf::internal::AuxillaryParseTableField aux[];
  static const ::google::protobuf::internal::ParseTable schema[1];
  static const ::google::protobuf::internal::FieldMetadata field_metadata[];
  static const ::google::protobuf::internal::SerializationTable serialization_table[];
  static const ::google::protobuf::uint32 offsets[];
};
void AddDescriptors();
}  // namespace protobuf_simpleResults_2eproto
namespace filebenchmark {
class SimpleResults;
class SimpleResultsDefaultTypeInternal;
extern SimpleResultsDefaultTypeInternal _SimpleResults_default_instance_;
}  // namespace filebenchmark
namespace google {
namespace protobuf {
template<> ::filebenchmark::SimpleResults* Arena::CreateMaybeMessage<::filebenchmark::SimpleResults>(Arena*);
}  // namespace protobuf
}  // namespace google
namespace filebenchmark {

// ===================================================================

class SimpleResults : public ::google::protobuf::Message /* @@protoc_insertion_point(class_definition:filebenchmark.SimpleResults) */ {
 public:
  SimpleResults();
  virtual ~SimpleResults();

  SimpleResults(const SimpleResults& from);

  inline SimpleResults& operator=(const SimpleResults& from) {
    CopyFrom(from);
    return *this;
  }
  #if LANG_CXX11
  SimpleResults(SimpleResults&& from) noexcept
    : SimpleResults() {
    *this = ::std::move(from);
  }

  inline SimpleResults& operator=(SimpleResults&& from) noexcept {
    if (GetArenaNoVirtual() == from.GetArenaNoVirtual()) {
      if (this != &from) InternalSwap(&from);
    } else {
      CopyFrom(from);
    }
    return *this;
  }
  #endif
  static const ::google::protobuf::Descriptor* descriptor();
  static const SimpleResults& default_instance();

  static void InitAsDefaultInstance();  // FOR INTERNAL USE ONLY
  static inline const SimpleResults* internal_default_instance() {
    return reinterpret_cast<const SimpleResults*>(
               &_SimpleResults_default_instance_);
  }
  static constexpr int kIndexInFileMessages =
    0;

  void Swap(SimpleResults* other);
  friend void swap(SimpleResults& a, SimpleResults& b) {
    a.Swap(&b);
  }

  // implements Message ----------------------------------------------

  inline SimpleResults* New() const final {
    return CreateMaybeMessage<SimpleResults>(NULL);
  }

  SimpleResults* New(::google::protobuf::Arena* arena) const final {
    return CreateMaybeMessage<SimpleResults>(arena);
  }
  void CopyFrom(const ::google::protobuf::Message& from) final;
  void MergeFrom(const ::google::protobuf::Message& from) final;
  void CopyFrom(const SimpleResults& from);
  void MergeFrom(const SimpleResults& from);
  void Clear() final;
  bool IsInitialized() const final;

  size_t ByteSizeLong() const final;
  bool MergePartialFromCodedStream(
      ::google::protobuf::io::CodedInputStream* input) final;
  void SerializeWithCachedSizes(
      ::google::protobuf::io::CodedOutputStream* output) const final;
  ::google::protobuf::uint8* InternalSerializeWithCachedSizesToArray(
      bool deterministic, ::google::protobuf::uint8* target) const final;
  int GetCachedSize() const final { return _cached_size_.Get(); }

  private:
  void SharedCtor();
  void SharedDtor();
  void SetCachedSize(int size) const final;
  void InternalSwap(SimpleResults* other);
  private:
  inline ::google::protobuf::Arena* GetArenaNoVirtual() const {
    return NULL;
  }
  inline void* MaybeArenaPtr() const {
    return NULL;
  }
  public:

  ::google::protobuf::Metadata GetMetadata() const final;

  // nested types ----------------------------------------------------

  // accessors -------------------------------------------------------

  // int32 status = 1;
  void clear_status();
  static const int kStatusFieldNumber = 1;
  ::google::protobuf::int32 status() const;
  void set_status(::google::protobuf::int32 value);

  // int32 inf_time_ms = 2;
  void clear_inf_time_ms();
  static const int kInfTimeMsFieldNumber = 2;
  ::google::protobuf::int32 inf_time_ms() const;
  void set_inf_time_ms(::google::protobuf::int32 value);

  // @@protoc_insertion_point(class_scope:filebenchmark.SimpleResults)
 private:

  ::google::protobuf::internal::InternalMetadataWithArena _internal_metadata_;
  ::google::protobuf::int32 status_;
  ::google::protobuf::int32 inf_time_ms_;
  mutable ::google::protobuf::internal::CachedSize _cached_size_;
  friend struct ::protobuf_simpleResults_2eproto::TableStruct;
};
// ===================================================================


// ===================================================================

#ifdef __GNUC__
  #pragma GCC diagnostic push
  #pragma GCC diagnostic ignored "-Wstrict-aliasing"
#endif  // __GNUC__
// SimpleResults

// int32 status = 1;
inline void SimpleResults::clear_status() {
  status_ = 0;
}
inline ::google::protobuf::int32 SimpleResults::status() const {
  // @@protoc_insertion_point(field_get:filebenchmark.SimpleResults.status)
  return status_;
}
inline void SimpleResults::set_status(::google::protobuf::int32 value) {
  
  status_ = value;
  // @@protoc_insertion_point(field_set:filebenchmark.SimpleResults.status)
}

// int32 inf_time_ms = 2;
inline void SimpleResults::clear_inf_time_ms() {
  inf_time_ms_ = 0;
}
inline ::google::protobuf::int32 SimpleResults::inf_time_ms() const {
  // @@protoc_insertion_point(field_get:filebenchmark.SimpleResults.inf_time_ms)
  return inf_time_ms_;
}
inline void SimpleResults::set_inf_time_ms(::google::protobuf::int32 value) {
  
  inf_time_ms_ = value;
  // @@protoc_insertion_point(field_set:filebenchmark.SimpleResults.inf_time_ms)
}

#ifdef __GNUC__
  #pragma GCC diagnostic pop
#endif  // __GNUC__

// @@protoc_insertion_point(namespace_scope)

}  // namespace filebenchmark

// @@protoc_insertion_point(global_scope)

#endif  // PROTOBUF_INCLUDED_simpleResults_2eproto
